...
let player1Matrix: number[][] = [
  [0, 0, 0],
  [0, 0, 0],
  [0, 0, 0]
];

let player2Matrix: number[][] = [
  [0, 0, 0],
  [0, 0, 0],
  [0, 0, 0]
];
...
function filterDuplicates(array: number[]): number[] {
...
function calculateScore(matrix: number[][]): number {
  let score = 0;

  // check each column
  for (let col = 0; col < 3; col++) {
    let diceInColumn: number[] = [];
...
function updateMatrix(player: number, column: number, dice: number): boolean {
